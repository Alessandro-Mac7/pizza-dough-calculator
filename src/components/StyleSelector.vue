...
<template>
  <section class="mb-8">
    <h2 class="text-2xl font-bold mb-4 text-wood dark:text-flour-yellow">1. Scegli lo Stile</h2>
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
      <button
        v-for="style in pizzaStyles"
        :key="style.id"
        class="relative p-4 rounded-xl text-left transition-all duration-300 border-2 cursor-pointer hover:shadow-lg hover:-translate-y-0.5 active:scale-[0.97]"
        :class="[
          selectedId === style.id
